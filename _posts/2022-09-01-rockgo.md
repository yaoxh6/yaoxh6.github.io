...
## actor
actor模型对于整个的架构作用在于所有的远程的调用会以`ActorMessageInfo`的形式存储在每个`ActorComponent`的`queueReceive chan *ActorMessageInfo //接收消息队列`,然后从queueReceive中取出消息处理。
...
在actor模型下,每个node默认添加`ActorProxyComponent`,`ActorProxyComnponent`会注册`ActorProxyService`服务,这样每个节点都有`Tell`和`ServiceInquiry`方法。
```go
this.Root().AddComponent(&Actor.ActorProxyComponent{})
```

没有Actor模型时调用远程方法,首先要通过`MasterService`或者`LocationService`得到对应的`rpcClient`,再通过`rpcClient`发起`Call`。使用Actor模型,一个Actor向另一个Actor发起请求则使用`Tell`,在Emit中通过nodeID判断是本地节点则走`LocalTell`,如果是远程则走网络代理。网络代理则是获取`rpcClient`然后发起`call`的方式。
```go
